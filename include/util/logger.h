    ...
    constexpr void log(LogLevel level, int line, const std::string& format = "{}",
        ...
        taskQueue_.emplace(
            [... args = std::forward<Args>(args), level, line, format, this]() {
                ...
                oss << "[" + std::string(dt) + "] [" + levelStr + "] ";
                oss << "[" + std::string(__FILE__) + ":" + std::to_string(line) + "]:"
                    << '\n';
                oss << fmt::format(format, std::move(args)...) << '\n';
                std::string message = oss.str();

                fwrite(message.data(), sizeof(char), message.size(), logFile_);
            });
    }
